...
export const CreateSpaceForm = ({ onSubmit, control, setValue, errors }) => {
  const { t } = useTranslation();

  const handleLogoUploadSuccess = useCallback(
    (result: any) => {
      // Set the logo URL from upload result
      const logoUrl = result?.download_url || result?.path || result?.url;
      if (logoUrl) {
  ...
  const formSections: FormSection[] = [
    {
      ...
      fields: [
        ...
        },
        {
          ...
          rules: {
            required: t('forms.input_required'),
            pattern: { value: /^[a-z0-9-]+$/, message: t('forms.slug_pattern') },
        ...
        },
        {
          ...
          options: [
            { label: t('space.types.private', 'Private'), value: 'private' },
            { label: t('space.types.public', 'Public'), value: 'public' },
            { label: t('space.types.internal', 'Internal'), value: 'internal' },
            { label: t('space.types.external', 'External'), value: 'external' },
            { label: t('space.types.other', 'Other'), value: 'other' }
          ],
      ...
      fields: [
        {
          ...
          description: t('space.title_hint', 'Title displayed in browser tab')
        },
        {
        ...
        },
        {
          ...
          description: t('space.logo_alt_hint', 'Alternative text for accessibility')
        }
      ]
    },
    {
      ...
      fields: [
        {
          ...
          className: 'col-span-full',
          rows: 3,
          rules: { maxLength: { value: 160, message: t('forms.max_length', { count: 160 }) } }
        },
        {
