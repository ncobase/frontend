...
import { useBulkUpdateUserSpaceRoles } from '../service';

interface SpaceUserBulkActionsProps {
  spaceId: string;
  selectedUsers: any[];
...
export const SpaceUserBulkActions: React.FC<SpaceUserBulkActionsProps> = ({
  spaceId,
  selectedUsers,
  onSelectionChange,
  onSuccess
  ...
  const handleBulkAction = () => {
    if (selectedUsers.length === 0) {
      toast.error(t('messages.error'), {
        description: t('space.users.bulk.no_selection')
      });
      return;
    }
  ...
  const onSubmit = async (data: any) => {
    try {
      ...
      toast.success(t('messages.success'), {
        description: t('space.users.bulk.success', { count: selectedUsers.length })
      });
      ...
      toast.error(t('messages.error'), {
        description: error['message'] || t('space.users.bulk.failed')
      });
    }
  };
  ...
  return (
    <>
        ...
        <div className='bg-white border border-slate-200/60 rounded-lg shadow-lg p-4'>
          <div className='flex items-center space-x-4'>
            <div className='flex items-center space-x-2'>
              ...
              <span className='text-sm font-medium'>{t('space.users.bulk.selected')}</span>
            </div>
            ...
            <div className='flex items-center space-x-2'>
              ...
              >
                ...
                {t('space.users.bulk.assign_roles')}
              </Button>
              ...
              >
                ...
                {t('space.users.bulk.remove_roles')}
              </Button>
      ...
      <Modal
        ...
        title={t(`space.users.bulk.${bulkAction}_title`)}
      ...
      {/* Confirmation Dialog */}
      <AlertDialog
        title={t('space.users.bulk.confirm_title')}
        description={t('space.users.bulk.confirm_description', {
          action: t(`space.users.bulk.${confirmDialog.action}`),
          ...
          <div className='space-y-4'>
            <div className='bg-blue-50 p-3 rounded-lg'>
              <p className='text-sm text-blue-800'>
                {t('space.users.bulk.assign_description', { count: selectedUsers.length })}
              </p>
            </div>
            ...
            <Controller
              ...
              render={({ field }) => (
                <SelectField
                  label={t('space.users.bulk.select_role')}
                  placeholder={t('space.users.bulk.select_role_placeholder')}
          ...
          <div className='space-y-4'>
            <div className='bg-yellow-50 p-3 rounded-lg'>
              <p className='text-sm text-yellow-800'>
                {t('space.users.bulk.remove_description', { count: selectedUsers.length })}
              </p>
            </div>
            ...
            <Controller
              ...
              render={({ field }) => (
                <SelectField
                  multiple
                  label={t('space.users.bulk.select_roles_to_remove')}
                  placeholder={t('space.users.bulk.select_roles_placeholder')}
                  options={roles.map(role => ({
                    label: role.name,
                    value: role.id
                  }))}
          ...
          <div className='space-y-4'>
            <div className='bg-purple-50 p-3 rounded-lg'>
              <p className='text-sm text-purple-800'>
                {t('space.users.bulk.update_description', { count: selectedUsers.length })}
              </p>
            </div>
            ...
            <div className='grid grid-cols-2 gap-4'>
              <Controller
                ...
                render={({ field }) => (
                  <SelectField
                    label={t('space.users.bulk.old_role')}
                    options={roles.map(role => ({
                      label: role.name,
                      value: role.id
                    }))}
              ...
              <Controller
                ...
                render={({ field }) => (
                  <SelectField
                    label={t('space.users.bulk.new_role')}
                    options={roles.map(role => ({
                      label: role.name,
                      value: role.id
                    }))}
    ...
    <div className='space-y-4'>
      ...
      <div className='bg-slate-50 p-3 rounded-lg'>
        <h4 className='text-sm font-medium mb-2'>
          {t('space.users.bulk.selected_users')} ({selectedUsers.length})
        </h4>
