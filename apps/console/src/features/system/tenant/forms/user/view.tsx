...
export const TenantUserViewerForm = ({ tenantId, userId }) => {
  const { t } = useTranslation();
  const { data: userTenantData, isLoading } = useQueryUserTenantRoles(tenantId, userId);
  ...
  const data = { ...userData, ...userTenantData };

  const viewerSections: ViewerSection[] = [
    {
      id: 'user_basic',
      title: t('tenant.users.section.user_basic', 'User Information'),
      subtitle: t('tenant.users.section.user_basic_subtitle', 'Basic user details'),
      icon: 'IconUser',
      fields: [
        {
          id: 'user_id',
        ...
        {
          id: 'phone',
          title: t('tenant.users.fields.phone', 'Phone'),
          accessor: 'phone'
        },
        {
          id: 'status',
          title: t('tenant.users.fields.user_status', 'User Status'),
          accessor: 'status',
          renderer: value => renderUserStatus(value, t)
        },
        {
          id: 'is_certified',
          title: t('tenant.users.fields.certified', 'Certified'),
    ...
    {
      id: 'tenant_roles',
      title: t('tenant.users.section.tenant_roles', 'Tenant Roles'),
      subtitle: t('tenant.users.section.tenant_roles_subtitle', 'Assigned roles in this tenant'),
      icon: 'IconUserCheck',
      fields: [
        {
          id: 'role_ids',
          title: t('tenant.users.fields.assigned_roles', 'Assigned Roles'),
          accessor: 'role_ids',
        ...
        {
          id: 'joined_at',
          title: t('tenant.users.fields.joined_at', 'Joined Tenant'),
          accessor: 'joined_at',
          renderer: value => formatDateTime(value) || t('tenant.users.unknown_join_date')
        },
        {
          id: 'is_active',
          title: t('tenant.users.fields.is_active', 'Active in Tenant'),
          accessor: 'is_active',
          renderer: value => renderTenantStatus(value, t)
        }
      ]
    },
    {
    ...
    {
      id: 'system_info',
      title: t('tenant.users.section.system_info', 'System Information'),
      subtitle: t('tenant.users.section.system_info_subtitle', 'System-generated data'),
      icon: 'IconDatabase',
      fields: [
        {
          id: 'user_created_at',
          title: t('tenant.users.fields.user_created_at', 'User Created'),
          accessor: 'created_at',
          renderer: value => formatDateTime(value)
        },
        {
          id: 'last_login',
          title: t('tenant.users.fields.last_login', 'Last Login'),
          accessor: 'last_login',
          renderer: value => formatDateTime(value) || t('tenant.users.never_logged_in')
        },
        {
          id: 'tenant_id',
          title: t('tenant.users.fields.tenant_id', 'Tenant ID'),
          accessor: 'tenant_id'
        },
        {
          id: 'relationship_id',
          title: t('tenant.users.fields.relationship_id', 'Relationship ID'),
...
const renderTenantStatus = (isActive: boolean, t: any) => {
  return isActive ? (
...
const renderRoles = (roleIds: string[], roles: any[], t: any) => {
  if (!roleIds || roleIds.length === 0) {
    return <span className='text-slate-500'>{t('tenant.users.no_roles')}</span>;
  }
