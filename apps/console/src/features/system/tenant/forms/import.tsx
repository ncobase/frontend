  ...
  const processImportData = useCallback(
    data => {
      try {
        ...
        throw new Error(t('tenant.import.invalid_format'));
      }
    },
    [t]
  );
    ...
    async data => {
      ...
      for (let i = 0; i < total; i++) {
        ...
        if (Math.random() > 0.1) {
          ...
          failed++;
          errors.push(`Item ${i + 1}: ${t('tenant.import.validation_error')}`);
        }
      ...
      <CardContent className='p-0'>
        ...
        >
          ...
          >
            <div className='space-y-4'>
              <Controller
                ...
                render={({ field }) => (
                  <SelectField
                    label={t('tenant.import.format', 'Import Format')}
                    {...field}
                    options={[
                      {
                        label: t('tenant.import.file_format', 'File Upload (CSV, JSON, Excel)'),
                        value: 'file'
                      },
                      {
                        label: t('tenant.import.json_format', 'JSON Data'),
                        value: 'json'
                      }
                    ]}
                  />
              ...
              {formatType === 'file' && (
                <Controller
                  ...
                  render={({ field }) => (
                    <UploaderField
                      label={t('tenant.import.file', 'Upload File')}
                      {...field}
                      accept='.json,.csv,.xlsx,.xls'
                      maxSize={10 * 1024 * 1024} // 10MB
                      description={t(
                        'tenant.import.file_hint',
              ...
              {formatType === 'json' && (
                <Controller
                  ...
                  render={({ field }) => (
                    <Textarea
                      label={t('tenant.import.json_data', 'JSON Data')}
                      {...field}
          ...
          <Section
            ...
            subtitle={t('tenant.import.options_description', 'Configure import behavior')}
            icon='IconSettings'
          >
            <div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
              <Controller
                ...
                render={({ field }) => (
                  <SelectField
                    label={t('tenant.import.conflict_strategy', 'On Conflict')}
                    {...field}
                    options={[
                      { label: t('tenant.import.strategy_skip', 'Skip existing'), value: 'skip' },
                      {
                        label: t('tenant.import.strategy_update', 'Update existing'),
                        value: 'update'
                      },
                      {
                        label: t('tenant.import.strategy_replace', 'Replace existing'),
                        value: 'replace'
                      }
                    ]}
                  />
          ...
          {previewData.length > 0 && (
            <Section
              title={t('tenant.import.preview_section', 'Data Preview')}
              subtitle={t('tenant.import.preview_description', 'Preview of data to be imported')}
              icon='IconEye'
            >
              <div className='space-y-3'>
                ...
                <div className='text-sm text-slate-500 text-center'>
                  {t('tenant.import.showing_preview', 'Showing first {{count}} items', {
          ...
          {isImporting && (
            <Section
              title={t('tenant.import.progress_section', 'Import Progress')}
              icon='IconProgress'
            >
              <div className='space-y-3'>
                <Progress value={importProgress} className='h-3' />
                <div className='text-center text-sm'>
                  {t('tenant.import.processing', 'Processing... {{progress}}%', {
          ...
          {importResult && (
            <Section
              title={t('tenant.import.results_section', 'Import Results')}
                ...
                <div className='grid grid-cols-3 gap-4'>
                  <div className='text-center p-3 bg-blue-50 rounded'>
                    ...
                    <div className='text-sm text-blue-800'>
                      {t('tenant.import.total_processed')}
                    </div>
                  </div>
                  <div className='text-center p-3 bg-green-50 rounded'>
                    <div className='text-2xl font-bold text-green-600'>{importResult.success}</div>
                    <div className='text-sm text-green-800'>{t('tenant.import.successful')}</div>
                  </div>
                  <div className='text-center p-3 bg-red-50 rounded'>
                    <div className='text-2xl font-bold text-red-600'>{importResult.failed}</div>
                    <div className='text-sm text-red-800'>{t('tenant.import.failed')}</div>
                  </div>
                </div>
                ...
                {importResult.errors.length > 0 && (
                  <Alert variant='warning'>
                    <div className='space-y-2'>
                      <div className='font-medium'>{t('tenant.import.errors_found')}</div>
                      <ul className='text-sm list-disc list-inside space-y-1'>
                        ...
                        {importResult.errors.length > 5 && (
                          <li>
                            {t('tenant.import.more_errors', 'And {{count}} more...', {
