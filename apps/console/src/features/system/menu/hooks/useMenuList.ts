...
import { useListMenus } from '../service';

export const useMenuList = (initialParams: QueryFormParams = { children: true }) => {
  const [queryParams, setQueryParams] = useState<QueryFormParams>(initialParams);
  const lastParamsRef = useRef(initialParams);

  const { data, isLoading, error, refetch } = useListMenus(queryParams);
