...
import { QueryFormParams } from '../config/user/query';
import { useQueryTenantUsers } from '../service';

export const useTenantUserList = (
  tenantId: string,
  initialParams: QueryFormParams = { limit: 20 }
) => {
  const [queryParams, setQueryParams] = useState<QueryFormParams>(initialParams);
  const lastParamsRef = useRef(initialParams);

  const { data, isLoading, error, refetch } = useQueryTenantUsers(tenantId, queryParams);
