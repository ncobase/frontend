...
import { useListOrgs } from '../service';

export const useOrgList = (initialParams: QueryFormParams = { children: true }) => {
  const [queryParams, setQueryParams] = useState<QueryFormParams>(initialParams);
  const lastParamsRef = useRef(initialParams);

  const { data, isLoading, error, refetch } = useListOrgs(queryParams);
