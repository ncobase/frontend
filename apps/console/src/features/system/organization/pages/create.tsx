...
import { CreateOrgForms } from '../forms/create';

import { useLayoutContext } from '@/components/layout';

export const CreateOrgPage = ({ viewMode, onSubmit, control, errors }) => {
  const { vmode } = useLayoutContext();
  const mode = viewMode || vmode || 'flatten';
  if (mode === 'modal') {
    return <CreateOrgForms onSubmit={onSubmit} control={control} errors={errors} />;
  }
  ...
  return (
    <>
      ...
      <ScrollView className='bg-white'>
        <Container>
          <CreateOrgForms onSubmit={onSubmit} control={control} errors={errors} />
        </Container>
      </ScrollView>
    </>
  );
