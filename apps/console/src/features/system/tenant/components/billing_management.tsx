...
import {
  ...
  useQueryTenantBilling,
  useCreateTenantBilling,
  useUpdateTenantBilling,
  useDeleteTenantBilling,
  useProcessPayment,
  useGenerateInvoice
} from '../service';

interface TenantBillingManagementProps {
  tenant: any;
  onNavigateToSettings?: (tenant: any) => void;
}

export const TenantBillingManagement: React.FC<TenantBillingManagementProps> = ({
  tenant,
  ...
  const { data: summary, refetch: refetchSummary } = useQueryBillingSummary(tenant?.id);
  const { data: billings, isLoading, refetch } = useQueryTenantBilling(tenant?.id);
  const createBillingMutation = useCreateTenantBilling();
  const updateBillingMutation = useUpdateTenantBilling();
  const deleteBillingMutation = useDeleteTenantBilling();
        ...
        <div className='flex items-center space-x-1'>
          {record.status === 'pending' && (
            <Button
              ...
              title={t('tenant.billing.pay_now')}
            >
          ...
          <Button
            ...
            title={t('tenant.billing.generate_invoice')}
          >
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      toast.success(t('messages.success'), {
        description: t('tenant.billing.delete_success')
      });
      ...
      toast.error(t('messages.error'), {
        description: error['message'] || t('tenant.billing.delete_failed')
      });
    }
  };
  ...
  const onSubmit = async (data: any) => {
    try {
      if (editingBilling) {
        ...
        toast.success(t('messages.success'), {
          description: t('tenant.billing.update_success')
        });
      } else {
        await createBillingMutation.mutateAsync(data);
        toast.success(t('messages.success'), {
          description: t('tenant.billing.create_success')
        });
      }
      ...
      toast.error(t('messages.error'), {
        description: error['message'] || t('tenant.billing.save_failed')
      });
    }
  };
  ...
  const onPaymentSubmit = async (data: any) => {
    try {
      ...
      toast.success(t('messages.success'), {
        description: t('tenant.billing.payment_success')
      });
      ...
      toast.error(t('messages.error'), {
        description: error['message'] || t('tenant.billing.payment_failed')
      });
    }
  };
  ...
  const handleGenerateInvoice = async () => {
    ...
    try {
      ...
      toast.success(t('messages.success'), {
        description: t('tenant.billing.invoice_generated')
      });
      ...
      toast.error(t('messages.error'), {
        description: error['message'] || t('tenant.billing.invoice_failed')
      });
    }
  };
  ...
  return (
    <div className='space-y-6'>
      ...
      <div className='flex items-center justify-between'>
        <div>
          <h3 className='text-lg font-medium'>{t('tenant.billing.title')}</h3>
          <p className='text-slate-600'>{t('tenant.billing.description')}</p>
        </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle className='flex items-center justify-between'>
            <span>{t('tenant.billing.invoice_list', 'Invoice Management')}</span>
            <Badge variant='outline-primary'>
              {billingList.length} {t('tenant.billing.total', 'total')}
            </Badge>
          </CardTitle>
        </CardHeader>
        <CardContent className='p-0'>
          {isLoading ? (
            ...
            <div className='text-center py-8 text-slate-500'>
              ...
              {t('tenant.billing.no_billings')}
              <div className='mt-4'>
                <Button onClick={handleCreate} variant='outline-primary'>
                  <Icons name='IconPlus' className='mr-2' />
                  {t('tenant.billing.create_first')}
                </Button>
              </div>
            </div>
          ) : (
      ...
      <Modal
        ...
        title={editingBilling ? t('tenant.billing.edit_title') : t('tenant.billing.create_title')}
      ...
      <Modal
        ...
        title={t('tenant.billing.process_payment')}
        confirmText={t('tenant.billing.pay_now')}
      ...
      <Modal
        ...
        title={t('tenant.billing.generate_invoice')}
        confirmText={t('tenant.billing.generate')}
        onConfirm={handleGenerateInvoice}
      >
        <div className='space-y-4'>
          <p>{t('tenant.billing.invoice_confirm')}</p>
          {invoiceDialog.billing && (
            <div className='bg-slate-50 p-4 rounded-lg'>
              <div className='grid grid-cols-2 gap-4 text-sm'>
                <div>
                  <span className='font-medium'>{t('tenant.billing.fields.invoice_number')}:</span>
                  <span className='ml-2'>{invoiceDialog.billing.invoice_number}</span>
                </div>
                <div>
                  <span className='font-medium'>{t('tenant.billing.fields.amount')}:</span>
                  <span className='ml-2'>
      ...
      {/* Delete Confirmation */}
      <AlertDialog
        title={t('tenant.billing.delete_confirm_title')}
        description={t('tenant.billing.delete_confirm_description')}
  ...
  <div className='space-y-4'>
    <div className='grid grid-cols-2 gap-4'>
      <Controller
        ...
        render={({ field }) => (
          <InputField
            label={t('tenant.billing.fields.invoice_number')}
      ...
      <Controller
        ...
        render={({ field }) => (
          <SelectField
            label={t('tenant.billing.fields.billing_period')}
            options={[
              { label: t('tenant.billing.periods.monthly'), value: 'monthly' },
              { label: t('tenant.billing.periods.yearly'), value: 'yearly' },
              { label: t('tenant.billing.periods.one_time'), value: 'one_time' },
              { label: t('tenant.billing.periods.usage_based'), value: 'usage_based' }
            ]}
      ...
      <Controller
        ...
        render={({ field }) => (
          <InputField
            type='number'
            label={t('tenant.billing.fields.amount')}
            placeholder='0.00'
      ...
      <Controller
        ...
        render={({ field }) => (
          <SelectField
            label={t('tenant.billing.fields.currency')}
            options={[
              { label: 'USD', value: 'USD' },
              { label: 'EUR', value: 'EUR' },
              { label: 'GBP', value: 'GBP' },
      ...
      <Controller
        ...
        render={({ field }) => (
          <SelectField
            label={t('tenant.billing.fields.status')}
            options={[
              { label: t('tenant.billing.status.pending'), value: 'pending' },
              { label: t('tenant.billing.status.paid'), value: 'paid' },
              { label: t('tenant.billing.status.overdue'), value: 'overdue' },
              { label: t('tenant.billing.status.cancelled'), value: 'cancelled' },
              { label: t('tenant.billing.status.refunded'), value: 'refunded' }
            ]}
      ...
      <Controller
        name='due_date'
        control={control}
        render={({ field }) => <DateField label={t('tenant.billing.fields.due_date')} {...field} />}
      />
    </div>
    ...
    <Controller
      ...
      render={({ field }) => (
        <InputField
          label={t('tenant.billing.fields.description')}
          placeholder={t('tenant.billing.placeholders.description')}
          {...field}
        />
      )}
    />
    ...
    <Controller
      ...
      render={({ field }) => (
        <SelectField
          label={t('tenant.billing.fields.payment_method')}
          options={[
...
const PaymentForm = ({ control, errors, billing, t }) => (
  <div className='space-y-4'>
    ...
    <Controller
      ...
      render={({ field }) => (
        <InputField
          label={t('tenant.billing.fields.transaction_id')}
          placeholder='TXN-123456789'
          description={t('tenant.billing.hints.transaction_id')}
          {...field}
        />
      )}
    />
