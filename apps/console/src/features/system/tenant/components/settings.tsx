...
import { updateTenant } from '../apis';
import {
  useQueryTenantSettings,
...
interface TenantSettingsProps {
  ...
  onNavigateToQuotas?: (tenant: any) => void;
  onNavigateToBilling?: (tenant: any) => void;
  onNavigateToView?: (tenant: any) => void;
  onNavigateToEdit?: (tenant: any) => void;
}
...
export const TenantSettings: React.FC<TenantSettingsProps> = ({
  ...
  // Data queries
  const { data: tenantSettings, refetch: refetchSettings } = useQueryTenantSettings(tenant?.id);
  const { data: tenantUsers } = useQueryTenantUsers(tenant?.id);
  const { data: tenantQuotas } = useQueryTenantQuotas(tenant?.id);
  const { data: billingSummary } = useQueryBillingSummary(tenant?.id);

  // Mutations
  const createSettingMutation = useCreateTenantSetting();
  const deleteSettingMutation = useDeleteTenantSetting();
  ...
  useEffect(() => {
    if (tenant && isOpen) {
      ...
      setValue('url', tenant.url);
      setValue('title', tenant.title);
      setValue('description', tenant.description);
      setValue('keywords', tenant.keywords);
      setValue('copyright', tenant.copyright);
      setValue('logo_alt', tenant.logo_alt);
      setValue('disabled', tenant.disabled);
      setValue('expired_at', tenant.expired_at);
      setValue('order', tenant.order);
    }
  }, [tenant, isOpen, setValue]);

  const onSubmit = useCallback(
    async data => {
      try {
        await updateTenant({ ...tenant, ...data });
        toast.success(t('messages.success'), {
          description: t('tenant.messages.update_success')
        });
        onSuccess?.();
        onClose();
      } catch (error) {
        toast.error(t('messages.error'), {
          description: error['message'] || t('tenant.messages.update_failed')
        });
      }
    },
    [tenant, toast, t, onSuccess, onClose]
  );
  ...
  const onSettingSubmit = useCallback(
    async data => {
      try {
        ...
        toast.success(t('messages.success'), {
          description: t('tenant.settings.setting_created')
        });
        ...
        toast.error(t('messages.error'), {
          description: error['message'] || t('tenant.settings.setting_create_failed')
        });
      }
    },
    [tenant?.id, createSettingMutation, toast, t, resetSetting, refetchSettings]
  );
  ...
  const handleDeleteSetting = useCallback(
    async (settingId: string) => {
      try {
        ...
        toast.success(t('messages.success'), {
          description: t('tenant.settings.setting_deleted')
        });
        refetchSettings();
      } catch (error) {
        toast.error(t('messages.error'), {
          description: error['message'] || t('tenant.settings.setting_delete_failed')
        });
      }
    },
  ...
  return (
    <>
      <Modal
        isOpen={isOpen}
        onCancel={onClose}
        title={t('tenant.settings.title', 'Tenant Management')}
        description={`${t('tenant.settings.description')} "${tenant.name}"`}
        ...
        <div className='space-y-6'>
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab}>
            <TabsList className='w-full justify-start mb-4'>
              <TabsTrigger value='general'>{t('tenant.settings.tabs.general')}</TabsTrigger>
              <TabsTrigger value='users'>{t('tenant.settings.tabs.users')}</TabsTrigger>
              <TabsTrigger value='quotas'>{t('tenant.settings.tabs.quotas')}</TabsTrigger>
              <TabsTrigger value='billing'>{t('tenant.settings.tabs.billing')}</TabsTrigger>
              <TabsTrigger value='custom'>{t('tenant.settings.tabs.custom')}</TabsTrigger>
            </TabsList>
            ...
            <TabsContent value='users' className='space-y-4'>
              <UsersManagementTab tenantId={tenant.id} users={tenantUsers} t={t} />
            </TabsContent>
            ...
            <TabsContent value='quotas' className='space-y-4'>
              <QuotasOverviewTab
                ...
                quotas={tenantQuotas}
                t={t}
                onNavigateToQuotas={() => onNavigateToQuotas?.(tenant)}
              />
            </TabsContent>
            ...
            <TabsContent value='billing' className='space-y-4'>
              <BillingOverviewTab
                ...
                t={t}
                onNavigateToBilling={() => onNavigateToBilling?.(tenant)}
              />
            </TabsContent>
      ...
      <Modal
        ...
        title={t('tenant.custom_settings.add_title')}
...
const TenantStatusBanner = ({ tenant, t, onNavigateToView, onNavigateToEdit }) => (
  <div
    className={`p-4 rounded-lg ${
      tenant.disabled ? 'bg-red-50 border border-red-200' : 'bg-green-50 border border-green-200'
    }`}
  >
    <div className='flex items-center justify-between'>
      <div className='flex items-center space-x-3'>
        <Icons
          name={tenant.disabled ? 'IconAlertTriangle' : 'IconCircleCheck'}
          className={tenant.disabled ? 'text-red-500' : 'text-green-500'}
        />
        <div>
          <div className='font-medium'>
            {tenant.disabled ? t('tenant.status.disabled') : t('tenant.status.active')}
          </div>
          <div className='text-sm text-slate-600'>
            {tenant.disabled
              ? t('tenant.status.disabled_description')
              : t('tenant.status.active_description')}
          </div>
        </div>
        {tenant.expired_at && tenant.expired_at > 0 && (
          <div className='text-sm text-slate-600'>
            {t('tenant.expires_at')}: {formatDateTime(tenant.expired_at, 'date')}
          </div>
        )}
      </div>
      ...
      <div className='flex items-center space-x-2'>
        <Button variant='link' size='xs' onClick={() => onNavigateToView?.(tenant)}>
          {t('actions.view')}
        </Button>
        <Button variant='link' size='xs' onClick={() => onNavigateToEdit?.(tenant)}>
          {t('actions.edit')}
        </Button>
      </div>
    </div>
...
const QuotasOverviewTab = ({ tenantId, quotas, t, onNavigateToQuotas }) => {
      ...
      <div className='grid grid-cols-1 md:grid-cols-2 gap-4 px-2'>
        <Card>
          ...
          <CardContent>
            ...
            <div className='text-xs text-slate-500 mt-1'>
              {t('tenant.quotas.configured_limits')}
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <div className='text-xs text-slate-500 mt-1'>{t('tenant.quotas.over_limit')}</div>
          </CardContent>
        </Card>
      </div>
                ...
                >
                  <div className='flex items-center justify-between mb-2'>
                    ...
                    <Badge variant={quota.is_exceeded ? 'danger' : 'success'}>
                      {quota.is_exceeded ? t('tenant.quotas.exceeded') : t('tenant.quotas.active')}
                    </Badge>
                  </div>
              ...
              {quotaList.length > 3 && (
                <div className='text-center py-2'>
                  <Button size='sm' variant='outline-slate' onClick={onNavigateToQuotas}>
                    {t('tenant.quotas.view_all', 'View all {{count}} quotas', {
...
const BillingOverviewTab = ({ tenantId, summary, t, onNavigateToBilling }) => (
  <div className='space-y-6'>
    {/* Billing Summary */}
    {summary && (
      <div className='grid grid-cols-1 md:grid-cols-4 gap-4 px-2'>
        ...
        <Card>
          <CardHeader className='pb-2'>
            <CardTitle className='text-sm font-medium text-slate-600'>
              {t('tenant.billing.pending_amount')}
            </CardTitle>
          </CardHeader>
          <CardContent>
        ...
        <Card>
          <CardHeader className='pb-2'>
            <CardTitle className='text-sm font-medium text-slate-600'>
              {t('tenant.billing.overdue_amount')}
            </CardTitle>
          </CardHeader>
          <CardContent>
            <div className='text-2xl font-bold text-red-600'>
    ...
    <Card>
      <CardHeader>
        <CardTitle className='flex items-center justify-between'>
          <span>{t('tenant.billing.quick_actions')}</span>
          <Button size='sm' variant='outline-primary' onClick={onNavigateToBilling}>
            <Icons name='IconArrowRight' className='w-4 h-4 mr-2' />
            {t('tenant.billing.manage_all')}
          </Button>
        </CardTitle>
      </CardHeader>
      <CardContent>
        <div className='grid grid-cols-1 md:grid-cols-3 gap-4 px-2'>
          ...
          >
            ...
            <div className='text-left'>
              <div className='font-medium'>{t('tenant.billing.create_invoice')}</div>
              <div className='text-xs text-slate-500'>
                {t('tenant.billing.create_invoice_desc')}
              </div>
            </div>
          </Button>
          ...
          >
            ...
            <div className='text-left'>
              <div className='font-medium'>{t('tenant.billing.record_payment')}</div>
              <div className='text-xs text-slate-500'>
                {t('tenant.billing.record_payment_desc')}
              </div>
            </div>
          </Button>
          ...
          >
            ...
            <div className='text-left'>
              <div className='font-medium'>{t('tenant.billing.view_history')}</div>
              <div className='text-xs text-slate-500'>{t('tenant.billing.view_history_desc')}</div>
            </div>
          </Button>
        </div>
      </CardContent>
...
const GeneralSettingsTab = ({ control, errors, t }) => (
  <div className='grid grid-cols-2 gap-4'>
    <Controller
      ...
      render={({ field }) => (
        <InputField
          ...
          placeholder={t('tenant.placeholders.name')}
    ...
    <Controller
      ...
      render={({ field }) => (
        <InputField
          ...
          placeholder={t('tenant.placeholders.slug')}
          disabled
          description={t('tenant.slug_readonly_hint')}
          {...field}
        />
      )}
    />
    ...
    <Controller
      ...
      render={({ field }) => (
        <SelectField
          label={t('tenant.fields.type')}
          options={[
            { label: t('tenant.types.private'), value: 'private' },
            { label: t('tenant.types.public'), value: 'public' },
            { label: t('tenant.types.internal'), value: 'internal' },
            { label: t('tenant.types.external'), value: 'external' },
            { label: t('tenant.types.other'), value: 'other' }
          ]}
          {...field}
        />
      )}
    ...
    <Controller
      ...
      render={({ field }) => (
        <InputField
          label={t('tenant.fields.url')}
          placeholder={t('tenant.placeholders.url')}
          {...field}
        />
      )}
    />
      ...
      <Controller
        ...
        render={({ field }) => (
          <Textarea
            ...
            placeholder={t('tenant.placeholders.description')}
            rows={3}
            {...field}
          />
        )}
    ...
    <div className='flex items-center justify-between p-4 bg-slate-50 rounded-lg col-span-2'>
      <div>
        <div className='font-medium'>{t('tenant.access.status')}</div>
        <div className='text-sm text-slate-600'>{t('tenant.access.status_description')}</div>
      </div>
      <Controller
        name='disabled'
        control={control}
    ...
    <Controller
      ...
      render={({ field }) => (
        <DateField
          label={t('tenant.fields.expired_at')}
          description={t('tenant.fields.expired_at_hint')}
          {...field}
        />
      )}
    />
    ...
    <Controller
      ...
      render={({ field }) => (
        <InputField
          type='number'
          label={t('tenant.fields.order')}
          placeholder='0'
          description={t('tenant.order_hint')}
          {...field}
        />
      )}
    />
...
const UsersManagementTab = ({ tenantId, users, t }) => {
      ...
      parser: value => {
        ...
        return (
          <Badge className={levelColors[value] || 'bg-slate-100'}>
            {t(`tenant.users.access_levels.${value}`)}
          </Badge>
        );
      }
    },
    {
      ...
      parser: value => (
        <Badge variant={value ? 'success' : 'danger'}>
          {value ? t('tenant.users.status.active') : t('tenant.users.status.inactive')}
        </Badge>
      )
    },
    {
  ...
  return (
    <div className='space-y-6'>
      ...
      <div className='grid grid-cols-1 md:grid-cols-3 gap-4 px-2'>
        <Card>
          <CardHeader className='pb-2'>
            <CardTitle className='text-sm font-medium text-slate-600 flex items-center'>
              ...
              {t('tenant.users.total_users')}
            </CardTitle>
          </CardHeader>
          <CardContent>
            <div className='text-2xl font-bold'>{userCount}</div>
            <div className='text-xs text-slate-500 mt-1'>{t('tenant.users.total_assigned')}</div>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <div className='text-xs text-slate-500 mt-1'>{t('tenant.users.currently_active')}</div>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <div className='text-xs text-slate-500 mt-1'>
              {t('tenant.users.administrator_level')}
            </div>
          </CardContent>
        </Card>
      </div>
      ...
      <Card className='shadow-none hover:shadow-none'>
        <CardHeader>
          <CardTitle className='flex items-center justify-between'>
            <span>{t('tenant.users.user_list', 'User List')}</span>
            <Button size='sm' variant='outline-primary'>
              <Icons name='IconPlus' className='w-4 h-4 mr-2' />
              {t('tenant.users.add_user')}
            </Button>
          </CardTitle>
        </CardHeader>
...
const CustomSettingsTab = ({ settings, onDelete, onAdd, t }) => {
  ...
  return (
    <div className='space-y-6'>
      <div className='flex items-center justify-between'>
        <div>
          <h4 className='font-medium'>{t('tenant.custom_settings.title')}</h4>
          <p className='text-sm text-slate-600'>{t('tenant.custom_settings.description')}</p>
        </div>
        <Button onClick={onAdd} size='sm'>
          <Icons name='IconPlus' className='mr-2' />
          {t('tenant.custom_settings.add')}
        </Button>
      </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle className='flex items-center justify-between'>
            <span>{t('tenant.custom_settings.list', 'Custom Settings')}</span>
            <Badge variant='outline-primary'>
              {settings.length} {t('tenant.custom_settings.total', 'total')}
            </Badge>
          </CardTitle>
        </CardHeader>
        <CardContent className='p-0'>
          {!settings || settings.length === 0 ? (
            <div className='text-center py-8 text-slate-500'>
              ...
              {t('tenant.custom_settings.no_settings')}
              <div className='mt-2'>
                <Button onClick={onAdd} size='sm' variant='outline-primary'>
                  <Icons name='IconPlus' className='mr-2' />
                  {t('tenant.custom_settings.add_first')}
                </Button>
              </div>
            </div>
          ) : (
  ...
  <div className='space-y-4'>
    <div className='grid grid-cols-2 gap-4'>
      <Controller
        ...
        rules={{
          ...
          pattern: {
            value: /^[a-z0-9_]+$/,
            message: t('tenant.custom_settings.key_pattern')
          }
        }}
        render={({ field }) => (
          <InputField
            label={t('tenant.custom_settings.fields.key')}
            placeholder='setting_key'
            error={errors.setting_key?.message}
            description={t('tenant.custom_settings.key_hint')}
            {...field}
          />
        )}
      />
      ...
      <Controller
        ...
        render={({ field }) => (
          <InputField
            label={t('tenant.custom_settings.fields.name')}
      ...
      <Controller
        ...
        render={({ field }) => (
          <SelectField
            label={t('tenant.custom_settings.fields.type')}
            options={[
      ...
      <Controller
        ...
        render={({ field }) => (
          <SelectField
            label={t('tenant.custom_settings.fields.scope')}
            options={[
    ...
    <div className='grid grid-cols-2 gap-4'>
      <Controller
        ...
        render={({ field }) => (
          <InputField
            label={t('tenant.custom_settings.fields.value')}
      ...
      <Controller
        ...
        render={({ field }) => (
          <InputField
            label={t('tenant.custom_settings.fields.default')}
    ...
    <Controller
      ...
      render={({ field }) => (
        <Textarea
          label={t('tenant.custom_settings.fields.description')}
      ...
      <Controller
        ...
        render={({ field: { value, onChange } }) => (
          <label className='flex items-center space-x-2'>
            ...
            />
            <span className='text-sm'>{t('tenant.custom_settings.fields.public')}</span>
          </label>
        )}
      />
        ...
        render={({ field: { value, onChange } }) => (
          <label className='flex items-center space-x-2'>
            <input
              ...
              className='rounded border-slate-300'
            />
            <span className='text-sm'>{t('tenant.custom_settings.fields.required')}</span>
          </label>
        )}
      />
        ...
        render={({ field: { value, onChange } }) => (
          <label className='flex items-center space-x-2'>
            <input
              ...
              className='rounded border-slate-300'
            />
            <span className='text-sm'>{t('tenant.custom_settings.fields.readonly')}</span>
          </label>
        )}
      />
    </div>
