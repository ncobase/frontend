...
import { CreateCommentForms } from '../forms/create';

import { useLayoutContext } from '@/layout';

export const CreateCommentPage = ({ viewMode, onSubmit, control, errors }) => {
  const { vmode } = useLayoutContext();
  const mode = viewMode || vmode || 'flatten';
  if (mode === 'modal') {
    return <CreateCommentForms onSubmit={onSubmit} control={control} errors={errors} />;
  }
  ...
  return (
    <>
      ...
      <ScrollView className='bg-white'>
        <Container>
          <CreateCommentForms onSubmit={onSubmit} control={control} errors={errors} />
        </Container>
      </ScrollView>
    </>
  );
