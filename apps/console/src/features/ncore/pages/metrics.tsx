...
import type { ExtensionMetrics } from '../ncore';
          ...
          left={[
            <div className='flex items-center gap-6 px-4 py-1.5'>
              <div className='flex items-center gap-2'>
                <Icons name='IconClock' className='w-4 h-4 text-slate-500' />
                <span className='text-slate-600'>Last updated: {currentTime.toLocaleString()}</span>
              </div>
              {autoRefresh.enabled && (
                <div className='flex items-center gap-2 px-3 py-1 bg-green-50'>
                  <div className='w-2 h-2 rounded-full bg-green-500 animate-ping' />
                  <span className='text-green-700'>Live updates every {refreshInterval}</span>
                </div>
              )}
            </div>
          ]}
          right={[
            <div className='flex items-center gap-3 p-1 bg-slate-50 rounded-lg'>
              <Select value={refreshInterval} onValueChange={setRefreshInterval}>
                <SelectTrigger className='w-24 py-1.5 bg-white'>
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
              ...
              <Tooltip content='Refresh current data'>
                <Button
