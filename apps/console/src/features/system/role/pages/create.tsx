...
import { CreateRoleForms } from '../forms/create';

import { useLayoutContext } from '@/layout';

export const CreateRolePage = ({ viewMode, onSubmit, control, errors }) => {
  const { vmode } = useLayoutContext();
  const mode = viewMode || vmode || 'flatten';
  if (mode === 'modal') {
    return <CreateRoleForms onSubmit={onSubmit} control={control} errors={errors} />;
  }
  ...
  return (
    <>
      ...
      <ScrollView className='bg-white'>
        <Container>
          <CreateRoleForms onSubmit={onSubmit} control={control} errors={errors} />
        </Container>
      </ScrollView>
    </>
  );
