...
interface TenantUserRoleManagementProps {
  isOpen: boolean;
  onClose: () => void;
  tenantId: string;
  user: any;
...
export const TenantUserRoleManagement: React.FC<TenantUserRoleManagementProps> = ({
  isOpen,
  onClose,
  tenantId,
  user,
  ...
  const { data: userRoles, isLoading, refetch } = useQueryUserTenantRoles(tenantId, user?.user_id);
  const { data: rolesData } = useListRoles({ limit: 100 });
  const addRoleMutation = useAddUserToTenantRole();
  const removeRoleMutation = useRemoveUserFromTenantRole();
  const updateRoleMutation = useUpdateUserTenantRole();
  ...
  const handleAddRole = async (data: any) => {
    try {
      ...
      toast.success(t('messages.success'), {
        description: t('tenant.users.roles.add_success')
      });
      ...
      toast.error(t('messages.error'), {
        description: error['message'] || t('tenant.users.roles.add_failed')
      });
    }
  };
  ...
  const handleRemoveRole = async () => {
    ...
    try {
      ...
      toast.success(t('messages.success'), {
        description: t('tenant.users.roles.remove_success')
      });
      ...
      toast.error(t('messages.error'), {
        description: error['message'] || t('tenant.users.roles.remove_failed')
      });
    }
  };
  ...
  return (
    <>
      <Modal
        isOpen={isOpen}
        onCancel={onClose}
        title={t('tenant.users.roles.manage_title')}
        description={`${t('tenant.users.roles.manage_description')} "${user.username}"`}
        className='max-w-4xl'
      ...
      >
        <div className='space-y-6'>
            ...
            <div className='flex items-center space-x-3'>
              ...
              <div className='ml-auto'>
                <Badge variant='outline-primary'>
                  {t('tenant.users.roles.roles_count', { count: assignedRoleIds.length })}
                </Badge>
              </div>
            </div>
          </div>
          ...
          {/* Current Roles */}
          <div>
            <h3 className='text-lg font-medium mb-4'>{t('tenant.users.roles.current_roles')}</h3>
          ...
          <div className='grid grid-cols-3 gap-4'>
            <div className='text-center p-3 bg-blue-50 rounded-lg'>
              ...
              <div className='text-sm text-blue-800'>{t('tenant.users.roles.assigned')}</div>
            </div>
            <div className='text-center p-3 bg-green-50 rounded-lg'>
              <div className='text-2xl font-bold text-green-600'>{availableRoles.length}</div>
              <div className='text-sm text-green-800'>{t('tenant.users.roles.available')}</div>
            </div>
            <div className='text-center p-3 bg-purple-50 rounded-lg'>
              <div className='text-2xl font-bold text-purple-600'>{roles.length}</div>
              <div className='text-sm text-purple-800'>{t('tenant.users.roles.total')}</div>
            </div>
          </div>
        </div>
      </Modal>
      ...
      <Modal
        ...
        title={t('tenant.users.roles.add_role_title')}
        confirmText={t('actions.add')}
        onConfirm={handleSubmit(handleAddRole)}
      >
        <div className='space-y-4'>
          <div className='bg-blue-50 p-3 rounded-lg'>
            <p className='text-sm text-blue-800'>{t('tenant.users.roles.add_role_description')}</p>
          </div>
          ...
          <Controller
            ...
            render={({ field }) => (
              <SelectField
                label={t('tenant.users.roles.select_role')}
                placeholder={t('tenant.users.roles.select_role_placeholder')}
          ...
          {availableRoles.length === 0 && (
            <div className='bg-yellow-50 p-3 rounded-lg'>
              <p className='text-sm text-yellow-800'>
                {t('tenant.users.roles.no_available_roles')}
              </p>
            </div>
          )}
        </div>
      ...
      <AlertDialog
        title={t('tenant.users.roles.remove_confirm_title')}
        description={t('tenant.users.roles.remove_confirm_description', {
  ...
  if (!role) {
    return (
      <div className='flex items-center justify-between p-3 border rounded-lg bg-red-50 border-red-200'>
        <div>
          <div className='font-medium text-red-800'>{t('tenant.users.roles.role_not_found')}</div>
          <div className='text-sm text-red-600'>
            {t('tenant.users.roles.role_not_found_description')}
          </div>
        </div>
    ...
    <div className='flex items-center justify-between p-3 border border-slate-200/60  rounded-lg hover:bg-slate-50'>
      <div className='flex-1'>
        ...
        <div className='text-xs text-slate-500 mt-1'>
          {t('tenant.users.roles.role_id')}: {role.id}
        </div>
      </div>
      <div className='flex items-center space-x-2'>
        <Button variant='outline-danger' size='xs' onClick={onRemove}>
