...
import { useListPermissions } from '../service';

export const usePermissionList = (initialParams: QueryFormParams = { limit: 20 }) => {
  const [queryParams, setQueryParams] = useState<QueryFormParams>(initialParams);
  const lastParamsRef = useRef(initialParams);

  const { data, isLoading, error, refetch } = useListPermissions(queryParams);
  ...
  return {
    data: data as Permissions | undefined,
    queryParams,
    loading: isLoading,
    error,
    fetchData,
